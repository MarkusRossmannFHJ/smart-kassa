...
describe("Register", () => {
  ...
  /**
   * Change the Username to firstname,lastname
   * @todo instead of username use firstname, lastname
   * @returns
   */
  const getFields = (
    placeholder: typeof authContent.register.placeholders
  ) => ({
  ...
  it("shows validation errors when inputs are blurred empty", async () => {
    ...
    const { email, atu, fn, phone, password } = getFields(placeholder);

    // username --> firsname & lastname
    const allInputs = [email, atu, fn, phone, password];
    ...
    // expect(
    //   await screen.findByText(validations.username.invalid)
    // ).toBeInTheDocument();
    expect(
      await screen.findByText(validations.email.invalid)
    ).toBeInTheDocument();
    expect(
      await screen.findByText(validations.atu.invalid)
    ).toBeInTheDocument();
    expect(await screen.findByText(validations.fn.invalid)).toBeInTheDocument();
    expect(
      await screen.findByText(validations.phone.invalid)
    ).toBeInTheDocument();

    for (const rule of Object.values(validations.password)) {
      expect(
        await screen.findByText(new RegExp(rule, "i"))
      ).toBeInTheDocument();
    }
  });
  ...
  it("hides validation errors after entering valid values", async () => {
    ...
    // username --> firsname & lastname
    for (const key of ["email", "atu", "fn", "phone"] as const) {
      await waitFor(() =>
        expect(
    ...
    for (const rule of Object.values(validations.password)) {
      await waitFor(() =>
        expect(
          screen.queryByText(new RegExp(rule, "i"))
        ).not.toBeInTheDocument()
      );
    }
  });
  ...
  it("enables the register button when placeholder values are typed", async () => {
    ...
    expect(
      screen.getByRole("button", { name: registerText })
    ).not.toBeDisabled();
  });
